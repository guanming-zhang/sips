#ifndef SIP_RANDOM_KICKER_CELL_LISTS_HPP
#define SIP_RANDOM_KICKER_CELL_LISTS_HPP
#include <algorithm>
// #include "sips/cell_list_unit_kicker.hpp"
#include "sips/cell_list_pairwise_random_kicker.hpp"
...
template <size_t ndim>
class CorrelatedPairwiseRandomKickerPeriodicCellLists : public ha::CellListPairwiseRandomKicker<CorrelatedUniformNoise,CorrelatedGaussianNoise,ha::periodic_distance<ndim> > {
public:
    const std::vector<double> m_boxv;
    const size_t m_ndim;
    CorrelatedPairwiseRandomKickerPeriodicCellLists(double kick_size, double correlation_mag, double correlation_dir,
                std::vector<double> const radii, std::vector<double> const boxv,
                const double ncellx_scale=1.0, const bool balance_omp=true)
        : CellListPairwiseRandomKicker<CorrelatedUniformNoise,CorrelatedGaussianNoise,ha::periodic_distance<ndim> >
        (std::make_shared<ha::periodic_distance<ndim> >(boxv),
        boxv,
        2.0 * (*std::max_element(radii.begin(), radii.end())), // rcut,
        ncellx_scale,
        ...
        m_ndim(ndim)
    {
        for (size_t i=0;i<this->m_kAcc.m_noises.size();i++){
            this->m_kAcc.m_noises[i]->set_correlation(correlation_mag);
        }
        for (size_t i=0;i<this->m_kAcc.d_noises.size();i++){
            this->m_kAcc.d_noises[i]->set_correlation(correlation_dir);
        }
    }
    void get_displacement(std::vector<double> const & coords, std::vector<double> & disp){
        CellListPairwiseRandomKicker<CorrelatedUniformNoise,CorrelatedGaussianNoise,ha::periodic_distance<ndim> >::get_kick(coords,disp);
    }
};
